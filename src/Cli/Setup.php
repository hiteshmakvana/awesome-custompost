...
namespace Awesome_Custom_Post\Cli;
...
class Setup {
	...
	public function __invoke( array $args, array $assoc_args ): void { // phpcs:disable Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
		...
		if (
			...
			|| ! $this->replace_in_files( 'awesome_custompost', str_replace( '-', '_', $this->slug ), array( '.*\.php' ) )
			|| ! $this->replace_in_files(
				'Awesome_Custom_Post',
				$this->namespace,
				array(
			...
			|| ! $this->replace_in_files( 'Awesome Custom Post', $this->name, array( '.*\.php', '.*README\.txt' ) )
		) {
	...
	private function rename_files(): void {
		...
		if (
			! rename( $this->path . '/src/Awesome_Custom_Post.php', $this->path . "/src/$this->namespace.php" ) // phpcs:disable WordPress.WP.AlternativeFunctions.rename_rename
			|| ! rename( $this->path . '/awesome-custompost.php', $this->path . "/$this->slug.php" ) // phpcs:disable WordPress.WP.AlternativeFunctions.rename_rename
		) {
